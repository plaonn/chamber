...
After official host registration contracts are validated against the installed CLIs, configure isolated native Codex and Gemini sessions, run audit-only coding tasks, and inspect the resulting local trace/evidence through `chamber trace` and `chamber evidence`. Orca can be used to launch or interact with those sessions, but it is only an optional interaction surface: Chamber requires no Orca-specific integration contract and must receive host events directly through its native adapters.
